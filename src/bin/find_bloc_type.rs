...
fn main() {
    ...
    let mut candidates = generate_candidates(ACTUAL_GRID);

    match_candidate_reference_template(&mut candidates, &templates);
}

fn _get_color(entry: &DirEntry) -> Vec<color_thief::Color> {
...
fn _get_color_2(img: &DynamicImage) -> Vec<color_thief::Color> {
    let img = image::open(entry.path()).unwrap();
    let rgb = img.to_rgb8();
    let pixels = rgb.as_raw();
...
pub fn _compare_distance(item_color: &Color, templates: &Vec<&Color>) -> usize {
...
fn generate_template(path: &str) -> Vec<Template> {
    ...
    for entry in template_dir.iter() {
        ...
        references.push(tmp_template)
    }
    references
}

fn generate_candidates(path: &str) -> Vec<Candidate> {
    let mut candidates = Vec::new();

    let candidate_dir: Vec<DirEntry> = std::fs::read_dir(path)
        .unwrap()
        .map(|entry| entry.unwrap())
        .collect();

    for entry in candidate_dir.iter() {
...
pub fn generate_candidates_2(screenshot: &DynamicImage, grid: &GridConfig) -> Vec<Candidate> {
    let mut candidates = Vec::new();
